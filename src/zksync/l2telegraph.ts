...
export default async function (hre) {
    ...
    console.log(`1. bridgemessage/....`)
    console.log(`2. mintNFT/....`)
    console.log(`3. bridgeNFT/....`)
    ...
    switch (nameFeature) {
        case '1': {
            console.log(`send msg:${msg} to ${dstChain}`)
            await bridgeMessage(dstChain,msg)
            break
        }
        case '2': {
...
const L2TELEGRAPH_MSG_ABI = require("../../abis/l2tele/send_message.json")
...
const chain_ids = {
    "CANTO": {'chain_id':CANTO_CHAINID,'msg_destination':CANTO_MSG_DESTINATION,'nft_destination':CANTO_NFT_DESTINATION},
...
async function getEstimateFee(web3:Web3,user_address,contract_address,destChainId){
    ...
    let l2_contract = new web3.eth.Contract(L2TELEGRAPH_MSG_ABI,contract_address);
    ...
    return Math.floor(fee[0] * 1.3)
}

const  assert  = require('assert')


async function bridgeMessage(destChain,msg) {
    ...
    console.log(chain_ids[destChain])
    console.log(`${account.address} tokenBalance before: ${formatUnits(eth_balance, 18)}`)

    let ABI = [
        "function sendMessage(string message, uint16 destChainId, bytes _destination)"
    ];
    let iface = new Interface(ABI);
    var endata = iface.encodeFunctionData("sendMessage", [msg,chain_ids[destChain].chain_id, chain_ids[destChain].msg_destination])
     console.log(endata)
    
    const tx_data = endata
    
    const router_address = L2TELEGRAPH_MSG_ADDRESS
    ...
    let l0_fee = await getEstimateFee(web3,account.address,L2TELEGRAPH_MSG_ADDRESS,chain_ids[destChain].chain_id)

    check.checkLoFee(l0_fee)

    let total_fee = BigNumber.from(l0_fee).add(parseEther('0.00025'))
    console.log(`l0_fee ${formatEther(l0_fee)} ETH ${formatEther(total_fee)}`);
